...
const Staff = () => {
    ...
    const getStaff = async () => {
        const docs = await getDocs(collection(db, 'conductor'))
        const list = []
    ...
    const delStaff = async (id) => {
        await deleteDoc(doc(db, 'conductor', id))
        getStaff()
    }

    const updateStaff = async (data) => {
        await updateDoc(doc(db, 'conductor', updateObj.id), data)
        getStaff()
        setUpdateObj(null)
    }
    const addStaff = async (dList) => {
        const doc = await addDoc(collection(db, 'conductor'), dList)
        getStaff()
    }
    return (
            ...
            <div className="busesDetails">
                ...
                <div className="controls">
                    {updateObj && <div className="busList row">
                        ...
                        <div className="details mh-8">{updateObj.phone}</div>
                        <div className="details mh-8 w100 t-center " title="cancel">
                    ...
                    <Forms className={'dataFields row'} onSubmit={updateObj ? updateStaff : addStaff}>
                        <Forms.Input placeholder="Name" defaultValue={updateObj?.name || ""} name={'name'} required />
                        <Forms.Input placeholder="Phone no" required defaultValue={updateObj?.phone || ""} type={'number'} name={'phone'} />
                        <button type="submit" className="btn primary">
                            <img src={check} alt="DONE" />
                        </button>
                    </Forms>
                </div>
                <div className="busList staffList">
                    {staffList.map((d, i) =>
